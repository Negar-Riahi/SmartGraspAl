...
for t in np.linspace(0,2*np.pi,1000):
	...
	cv2.circle(overlay,(y,x),1,(255,0,0))
	...
	for k in [-1,-0.5,0,0.5,1]:
		...
		x=int(cx+xyz[0]/-xyz[2]*focus)
		y=int(cy+xyz[1]/-xyz[2]*focus)
		cv2.circle(overlay,(y,x),0,(0,0,255))
...
while True:
	show=img_in.copy()
	cv2.rectangle(show,(int(xy0[1]),int(xy0[0])),(int(xy1[1]),int(xy1[0])),(0,0,255))
	for x,y in [xy0,xy1]:
		cv2.circle(show,(int(y),int(x)),5,(0,0,255),-1)
		cv2.circle(show,(int(y),int(x)),4,(255,255,255),-1)
	x0,y0=xy0
	x1,y1=xy1
	...
	#test=overlay*(overlay_mask) #just the sae sphere only colored

	#show_cropped=((cropped-(cropped/2*cropped_mask[:,:,None]))*(~overlay_mask))#|(overlay*(overlay_mask))
	#show_cropped=((cropped-(cropped/2*cropped_mask[:,:,None]))*(~overlay_mask))|test
	show_cropped=cropped*(~overlay_mask)
	cv2.imshow('image',show)
	#cv2.imshow('cropped',show_cropped)
	#cv2.imshow('cropped',cropped)
	cv2.imshow('show_cropped',show_cropped)
	...
	if cmd==ord('s'):
		...
		#cv2.imwrite(r'E:\N\Arshad\6TH\KIT_images\%s_crop.png'%sys.argv[1],cropped)
		#cv2.imwrite(r'E:\N\Arshad\6TH\KIT_images\%s_crop_m.png'%sys.argv[1],np.uint8(cropped_mask)*255)
		address1=r'E:\N\Arshad\6TH\KIT_images'+'\\'+str(number)+'_crop.png'
		address2=r'E:\N\Arshad\6TH\KIT_images'+'\\'+str(number)+'_crop_m.png'
		cv2.imwrite(address1,cropped)
		cv2.imwrite(address2,np.uint8(cropped_mask)*255)
